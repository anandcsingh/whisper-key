...
import type { ZkappWorkerRequest, ZkappWorkerReponse, WorkerFunctions } from './ProveBjjRankWorker';
import { ProveZkClient, ZkClientResponse } from '../../../../../contracts/build/src/models/RankProofClients';
export default class ProveBjjRankWorkerClient implements ProveZkClient {
...
{
  await this._call('proveRank', { prover, inquirer, rank });
}
async updateBackingStore(prover: string,
  inquirer: string,
  rank: string,): Promise<void>
{
  console.log("no updates to backing store");
}
  ...
  constructor() {
    this.worker = new Worker(new URL('./ProveBjjRankWorker.ts', import.meta.url))
    this.promises = {};
    this.nextId = 0;
